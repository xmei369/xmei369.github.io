...
p(y)与p(y/x)的区别：p(y)是全部训练组中的y取值的概率分布，并扩展到整个实数域（或(0,1)域等）。p(y/x)是当x的取值可以让y充满y的取值域时，p(y/x)=p(y)，若不能，就是p(y)的一部分。如果所有的x域可以使y取满所有值，那么所有的p(y/x)之和：sum(p(y/x))=p(y)。x分割了、限定了y的取值空间，进而求出在某局部取值空间内的次数概率。在高斯分布中，每个样本的p(y^(i)/x^(i))都不一样，除非有两个样本的特征值都一样。为什么每一个样本都可以推出一个概率分布？

在判别法中，

p(y/x)的期望均方是h(x)，所以与x和h（也就是参数θ）的变化有关。注意：与σ无关，因为每个样的σ都一样。

也就是说，这里的y的期望均方不是由y的训练数据集决定的，每一个y^(i)的期望均方是由假定的函数得出的y的预测值决定的。其实这里求的是误差值的概率分布。所以p(y/x)其实就是p(ε)的平移。

在生成法中，

p(y)是训练组中的y的概率分布，期望均方只跟y有关，与x等无关。

也就是说，这里的y的期望均方，真的就是训练数据集里所有的y推广到其取值域的期望均方。而不是误差值p(ε)的概率分布了。

p(x/y)是每个y类别中的x的概率分布。也是一部分y对应的x的训练数据推广到整个取值域的概率分布。这里的y到x是一对多的映射了，不算函数了。